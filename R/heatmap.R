#' @export
heatmap.plot.levels <- function(z, nlevels=10) {
...
#' @param nx,ny the number of bins in x and y dimension, used by \code{\link{vbin_grid}}.
#' @param na replacemnet for NA value in matrix bins, used by \code{\link{vbin_grid}}.
...
#' @export
heatmap.plot <- function(x, y, z, nx=100, ny=100, na=0, nlevels=10, levels=NULL,
  ...
  if(is.null(levels)) {
    ...
    if(is_empty(na))
      mv <- mv[!is_empty(mv)]
    levels <- heatmap.plot.levels(mv, nlevels)
  }
